    ...
    function ($, ko, kom, _, locale) {
        ...
        function myModel1() {
            ...
            self.viewModel = {
                ...
                entity: {
                    _Dutys: ko.observableArray(),//表单职务数据
                    DutyId: ko.observable(),
                    State: ko.observable("true"),
                    IsReturnEmployees: ko.observable("true")
                },
                mySave: function () {
            ...
            //获取部门并绑定对应控件
            self.initDepartment = function () {
                //控件适配器
                myModel.find("#DepartmentId").jqxTree({ width: 200, height: 220 });
            }
            //获取人员信息
            self.initPersonnel = function () {
                var personnelSource =
                {
                    type: "GET",
                    datatype: "json",
                ...
                var personnelDataAdapter = new $.jqx.dataAdapter(personnelSource, {
                    downloadComplete: function (data) {
                        var result = JSON.parse(data.Entity);
                        return result.data;
                    }
                });
                myModel.find("#PersonnelId").jqxGrid(
                    {
                        localization: locale.localizationobj,
                        width: 500,
                        pagermode: "simple",
                        pageable: true,
                        source: personnelDataAdapter,
                        virtualmode: true,
                        rendergridrows: function (obj) {
                            return obj.data;
                        },
                        columns: [
                            { text: '姓名', columntype: 'textbox', filtercondition: 'starts_with', datafield: 'name' },
                        ]
                    });
            }
            self.init = function () {
                ...
                myModel.on("shown.bs.modal", function () {
                    ...
                    myModel.find("#EntryTime").daterangepicker({ "singleDatePicker": true });
                    self.initDepartment();
                    self.initPersonnel();
