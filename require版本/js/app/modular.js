    ...
    function ($, ko, kom, _, locale) {
            ...
            self.init = function (fn) {
                ...
                var contextMenu = $("#Menu").on('itemclick', function (event) {
                    ...
                    if ($.trim($(args).text()) == "编辑") {
                        ...
                        _myModel1.show("编辑");
                    } else if ($.trim($(args).text()) == "添加子模块") {
                        _myModel1.viewModel.entity.ParentModularId(entity.ParentModularId);
                        _myModel1.viewModel.entity.ParentModularName(entity.ParentModularName);
                        _myModel1.show("新增");
                    }
                ...
                myGrid.on('contextmenu', function () {
                ...
                }).jqxGrid(
                    {
                        ...
                        columns: [
                            { text: '模块编号', datafield: 'ModularID' },
                            { text: '上级模块', datafield: 'ParentModularID' },
                            { text: '模块名称', datafield: 'ModularName' },
                            { text: '模块URL', datafield: 'ModularURL' },
                            { text: '模块备注', datafield: 'ModularRemark' },
                            { text: '模块创建时间', datafield: 'CreateTime' },
                            { text: '模块修改时间', datafield: 'EditTime' },
                            { text: '模块状态', datafield: 'Status' },
                            { text: '创建时间', datafield: 'CreateDate' },
                            { text: '修改时间', datafield: 'ModifyDate' },
                            { text: '删除标志', datafield: 'DeleteMark' },
                            { text: '有效标志', datafield: 'EnabledMark' },
                            { text: '修改用户主键', datafield: 'ModifyUserId' },
                            { text: '修改用户信息', datafield: 'ModifyUserName' }
                        ]
                    });
        ...
        function myModel1() {
            ...
            self.viewModel = {
                ...
                entity: {
                    ParentModularName: ko.observable("顶级模块"),
                    ParentModularID: ko.observable(0),
                    ModularName: ko.observable(),
                    ModularURL: ko.observable(),
                    Status: ko.observable("true"),
                    ModularRemark: ko.observable()
                },
                mySave: function () {
